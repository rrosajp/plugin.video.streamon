    ...
    def __callRequest(self):
        ...
        try:
            ...
            if e.code == 503 and e.headers.get("Server") == 'cloudflare-nginx' or 'cloudflare':
                ...
                xbmcgui.Dialog().ok('streamon', 'Fehler beim Abrufen der Url:', self.__sUrl, str(e))
            ...
            if not self.ignoreErrors:
                if hasattr(e.reason, 'args') and e.reason.args[0] == 1 and sys.version_info < (2, 7, 9):
                    xbmcgui.Dialog().ok('streamon', str(e.reason), '','For this request is Python v2.7.9 or higher required.')
                else:
                    xbmcgui.Dialog().ok('streamon', str(e.reason))
            logger.error("URLError " + str(e.reason) + " Url: " + self.__sUrl)
            return ''
        except httplib.HTTPException, e:
            if not self.ignoreErrors:
                xbmcgui.Dialog().ok('streamon', str(e))
