...
class cHosterGui:
        ...
        try:
            ...
            if siteResult:
                ...
                if mediaUrl:
                    ...
                    oGui.showError('streamon', 'kein Hosterlink übergeben', 5)
                    return False
            elif mediaUrl:
                logger.info('resolve: ' + mediaUrl)
            ...
            oGui.showError('streamon', 'Stream nicht mehr verfügbar oder Link fehlerhaft', 7)
            return False
    ...
    def stream(self, playMode, siteName, function, url):
        ...
        self.dialog.create('streamon', "get stream/hoster")
        ...
        if not siteResult:
            self.dialog.close()
            cGui().showInfo('streamon', 'stream/hoster not available')
            return
        ...
        if 'name' in siteResult[0]:
            ...
            if not siteResult:
                self.dialog.close()
                cGui().showInfo('streamon', 'no hoster available')
                return
            ...
            if not siteResult:
                self.dialog.close()
                cGui().showInfo('streamon', 'no supported hoster available')
                return False
    ...
    def streamAuto(self, playMode, siteName, function):
        ...
        if 'name' in siteResult[0]:
            ...
            if len(siteResult) > self.maxHoster:
                siteResult = siteResult[:self.maxHoster - 1]
            check = False
            self.dialog.create('streamon', 'try hosters...')
            total = len(hosters)
