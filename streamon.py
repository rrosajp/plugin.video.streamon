...
def updateMeta(params):
    ...
    if mediaType == 'movie' or mediaType == 'tvshow':
        ...
        if not foundInfo:
            oGui.showInfo('streamon', 'Suchanfrage lieferte kein Ergebnis')
            return
...
#        return
    ...
    if params.exist('function'):
        ...
        if sFunction == 'spacer':
            ...
            searchAlter(params)
            return
        elif sFunction == 'updatestreamon':
            from resources.lib import updateManager
            updateManager.streamonUpdate()
            return
    ...
    if not params.exist('site'):
        ...
        if streamonUpdate and urlResolverUpdate:
            ...
            xbmc.executebuiltin('XBMC.RunPlugin(%s?function=updatestreamon)' % sys.argv[0])
...
def settingsGuiElements():
    ...
    if cConfig().getSetting('metahandler') == 'true':
        return streamonSettings, urlResolverSettings, metaSettings
